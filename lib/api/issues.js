 ...
 *  - to: revision ID (int) or ISO timestamp (string) [optional, defaults to latest]
 *  - order: 'asc' (default) or 'desc' — sorting order of full revision trail
 *
 * Response:
 * {
 *   success: true,
...
Issues.compareRevisions = async (ctx) => {
  ...
  try {
    // Determine default 'to' if missing
    if (!toParsed) {
    ...
    // Ensure from and to are of same type
    ...
    if (fromParsed.value === toParsed.value) {
      return respond.badRequest(ctx, ['`from` and `to` must be different']);
    }

    // Prepare condition for edge revisions query
    ...
    // Build condition for full revision trail between from and to
    const trailWhere = { issueId };
    if (fromParsed.type === 'revisionNumber') {
      const [start, end] = fromParsed.value < toParsed.value ? [fromParsed.value, toParsed.value] : [toParsed.value, fromParsed.value];
