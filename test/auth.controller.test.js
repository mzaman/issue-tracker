...
describe('Auth Controller - login', () => {
    ...
    test('returns token and user on successful login', async () => {
        ctx.request.body = { email: 'test@example.com', password: 'pass' };
        UserMock.findOne.resolves({ id: 1, name: 'Admin', email: 'test@example.com', password_hash: 'hashedPass' });
        ...
        expect(ctx.body).toHaveProperty('message', 'Request succeeded');
        expect(ctx.body.data).toHaveProperty('token', 'mockedToken');
        expect(ctx.body.data.user).toMatchObject({
            id: 1,
            name: 'Admin',
